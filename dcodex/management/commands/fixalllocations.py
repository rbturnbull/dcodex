...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for location in VerseLocation.objects.filter(deck_membership=None):
            ...
            deck_membership = DeckMembership.objects.filter(deck=location.manuscript.imagedeck, image=image.deckimagefiler).first()
        ...
        for page in Page.objects.all():
            ...
            image = FilerImage.objects.filter(original_filename=filename).first()
            deck_membership = DeckMembership.objects.filter(image=image.deckimagefiler).first()

            print(page)
            print(filename, deck_membership)
            FolioRef.objects.update_or_create( deck_membership=deck_membership, folio=page.folio, side=page.side)
