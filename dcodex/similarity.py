...
def plot_rolling_average( 
    ...
    if not force_compute and csv_filename and isfile( csv_filename ) and access(csv_filename, R_OK):
        ...
        df = manuscript.rolling_average_probability( mss=mss, transcriptions=transcriptions, gotoh_param=gotoh_param, weights=weights, prior_log_odds=0.0 )
        if csv_filename:
    ...
    for index, ms_siglum in enumerate(mss_sigla.keys()):    
        if mode is HIGHLY_LIKELY__LIKELY__ELSE:
            ...
            plt.plot(df.index, df[ms_siglum+'_similarity'].mask(df[ms_siglum+"_probability"] < 0.5), '-', color=colors[index], linewidth=2.5, label=mss_sigla[ms_siglum] + " (Likely)" );
            plt.plot(df.index, df[ms_siglum+'_similarity'], '--', color=colors[index], linewidth=1, label=mss_sigla[ms_siglum] + " (Unlikely)" );        
#            plt.plot(df.index, df[ms_siglum+'_similarity'].mask(df[ms_siglum+"_probability"] > 0.5), '--', color=colors[index], linewidth=1, label=mss_sigla[ms_siglum] + " (Unlikely)" );        
    ...
    for annotation in annotations:
        ...
        ax.annotate(annotation_description, xy=(verse.id, ymax), xycoords='data', ha='center', va='bottom',xytext=(0,10), textcoords='offset points', fontsize=10, family='Linux Libertine O', color=annotation_color)

    ax.legend(shadow=False, title='', framealpha=1, edgecolor='black', loc=legend_location, facecolor="white")    
