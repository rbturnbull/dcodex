...
def filter_transcriptions_viewable( transcriptions, user ): 
    return [transcription for transcription in transcriptions if transcription.manuscript.has_view_permission( user )]

def filter_manuscripts_viewable( manuscripts, user ): 
    manuscripts = [manuscript for manuscript in manuscripts if manuscript.has_view_permission( user )]
...
#    $dcodex->comparisonTableForGroups($manuscriptID, $verseID, $columns );
    ...
    renders = []
    reference_comparison_texts = filter_transcriptions_viewable( manuscript.comparison_texts( verse, reference_mss ), request.user )
    profile_reference = template.render({'heading': "Reference", 'comparison_texts': reference_comparison_texts}, request)
    renders.append( profile_reference )
    
    families = manuscript.families_at(verse)
    for family in families:        
        family_transcriptions = filter_transcriptions_viewable( family.transcriptions_at( verse ), request.user )
        renders.append( template.render({'heading': family.name, 'comparison_texts': family_transcriptions}, request) )
        ...
        if len(untranscribed_manuscript_ids) > 0:
            ...
            untranscribed_manuscripts = [manuscript for manuscript in untranscribed_manuscripts if manuscript.has_view_permission( request.user )]
            renders.append( ms_hover.render( {'heading': 'Untranscribed', 'manuscripts': untranscribed_manuscripts, 'verse':verse}, request) )
        
    all_transcriptions = filter_transcriptions_viewable( manuscript.comparison_texts( verse ), request.user )
    all_mss = template.render({'heading': "All Transcribed", 'comparison_texts': all_transcriptions}, request)
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["manuscripts_with_decks"] = filter_manuscripts_viewable(Manuscript.objects.exclude(imagedeck=None), self.request.user)
